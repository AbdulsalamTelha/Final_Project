...
class StudentResource(ModelResource):
    ...
    #                 if not course.departments.filter(name=department).exists():
    ...
    #     # تحقق من وجود الحقول الإضافية المتعلقة بالمواد
    #     if not row.get('course_status') or not row.get('course_semester') or not row.get('course_year'):
    #         raise ValueError("All fields related to 'StudentCourse' (status, semester, year) are required.")
